...
public class Parser {
    ...
    private final Map<String, Counter<String>> htmlTag = Map.of(
            "*", new Counter<>("em>"),
            "_", new Counter<>("em>"),
            "**", new Counter<>("strong>"),
            "__", new Counter<>("strong>"),
            "--", new Counter<>("s>"),
            "`", new Counter<>("code>"),
            "++", new Counter<>("u>")
    );
    ...
    public void makeTag(int lvl, String prefix) {
        ...
        if (lvl > 0) {
            stringBuilder.append('h').append(lvl);
        } else {
    ...
    public void Parse(String fileIn, String fileOut) throws IOException {
        ...
        new Writer(fileOut).write(strings);
    }
    ...
    private void ParseString(StringBuilder stringBuilder, String str) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            Counter<String> pairs = htmlTag.get(substring);
            if (htmlTag.containsKey(substring) && pairs.getSecond() > 1) {
                pairs.minusCount();
                stringBuilder.append('<');
                if (pairs.getSecond() % 2 == 1) {
                    stringBuilder.append('/');
                }
                stringBuilder.append(pairs.getFirst());
    ...
    private void counter(int j) {
        ...
        for (String tag : htmlTag.keySet()) {
            if (htmlTag.get(tag).getSecond() % 2 == 0) {
